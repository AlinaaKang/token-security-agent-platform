...
export function PcapDetectionWorkspace() {
  ...
  return <section className="pcap-detection-workspace" aria-label="PCAP 异常检测工作区" aria-busy={busy}>
    ...
    {mission ? <><section className={`pcap-detection-status is-${mission.status}`}><strong>{mission.status === "completed" ? "检测完成" : mission.status === "cancelled" ? "检测已取消" : mission.status === "degraded" ? "检测降级完成" : "检测运行中"}</strong><code>{mission.detection_id}</code>{active ? <button type="button" onClick={cancel} disabled={busy}><Square size={13} />取消</button> : null}</section><div className="pcap-detection-findings"><div><strong>局部证据</strong>{mission.summary?.evidence.length ? mission.summary.evidence.map((item) => <article key={item.evidence_id}><span>{candidateLabels[item.attack_candidate]}</span><code>{item.evidence_id}</code><small>Packet {item.start_packet}-{item.end_packet} · {item.start_offset_ms}ms · 置信度 {Math.round(item.confidence * 100)}%</small></article>) : <p>证据不足，保持允许。</p>}</div><DetectionTimeline evidence={mission.summary?.evidence ?? []} /></div><DetectionMascots mission={mission} /></> : <div className="pcap-detection-empty"><ArrowRight size={22} /><strong>规则侦探等待授权</strong><span>检测结果只展示局部证据，不恢复原始请求。</span></div>}
  </section>;
}
