import { Activity, BadgeCheck, Network, ShieldCheck, Square } from "lucide-react";
...
function DetectionMascots({ mission }: { mission: PcapDetectionMissionResult }) {
  const first = mission.summary?.evidence[0];
  return <section className="pcap-detection-mascots" aria-label="PCAP 检测小队"><header><strong>检测小队</strong><span>每位公仔只汇报公开证据</span></header><div className="pcap-detection-mascot-grid"><article><ShieldCheck aria-hidden="true" size={22} /><strong>规则侦探</strong><span>{first ? candidateLabels[first.attack_candidate] : "等待规则结果"}</span></article><article><Activity aria-hidden="true" size={22} /><strong>序列侦探</strong><span>{first?.granularity === "request" ? "短请求无需调用 CPD" : "等待序列证据"}</span></article><article><BadgeCheck aria-hidden="true" size={22} /><strong>小队队长</strong><span>{first ? `已锁定 Packet ${first.start_packet}` : "证据不足，不强行报警"}</span></article></div></section>;
}
...
export function PcapDetectionResult({ mission, sampleLabel, onCancel, busy }: PcapDetectionResultProps) {
  ...
  return <>
    ...
    <DetectionMascots mission={mission} />
  </>;
}
