...
- Prompt 调查结果、PCAP 单文件结果、数据勘察、批量分诊和异常检测均提供就近的结果解释；空表单保持简洁。
- 打开或关闭说明不会提交 Prompt、创建任务、上传文件、签发授权、运行 Docker 或执行工具。
- Lab 与 PCAP 隐私验证共 59 项通过；完整前端回归为 31 个测试文件、270 个用例通过，生产构建成功。
- 桌面 `1440x900` 与手机 `390x844` 检查无全局横向溢出、控制台错误或页面错误。

`/lab?surface=prompt` 与 `/lab?surface=pcap` 是两个固定输入类型的专业实验入口。Prompt 页面复用现有语义 Guard、Entropy-CPD、固定融合和本地知识证据；PCAP 页面接收一个由用户明确选择的 `.pcap` 或 `.pcapng` 文件，并复用现有 Docker 隔离检测器。页面内部不再提供跨类型切换，避免 Prompt 与网络抓包工作流混在一起；旧 `/lab` 继续默认进入 Prompt 实验。

首次进入实验舱会显示操作浮窗；完成后可通过右下角“本页引导”重开。Prompt 或 PCAP 形成结果后，结果旁的“如何理解”折叠区解释状态含义。打开这些说明不会提交 Prompt、上传文件、签发授权或执行工具。
...
阅读结果时必须区分三类状态：异常候选表示已有证据命中；当前范围未命中表示现有可见证据没有命中；工具失败表示没有形成可验证结果。工具失败既不能计为安全，也不能计为异常。反事实敏感性只描述受控改动前后的响应变化，不构成严格因果证明。

Prompt 游戏化入口、50/20/30 评分、录制回放、隐私边界和演示步骤见 [Token 侦探挑战](token-detective-challenge.md)。`/pcap-challenge` 则复用真实 PCAP 异常检测、Packet 定位和检测小队，不使用虚构分数。两个挑战均不改变专业检测流程或阈值。
...
路径一在 `/lab?surface=prompt` 中使用普通无害和无害格式突变，说明“分布变化不自动等于恶意”。路径二使用直接危险样本，说明语义 Guard 覆盖无明显后缀的内容风险。路径三依次选择 GCG、AutoDAN、AdvPrompter 的受保护 ID，展示 Token 定位、反事实敏感性、知识引用、模拟处置和脱敏报告。路径四打开 `/lab?surface=pcap`，选择一个合成 PCAP，完成两次确认后展示上传进度、Docker 隔离边界和局部异常证据。
