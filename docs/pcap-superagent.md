...
总体分布或安全结论。画像默认抽取 100 个代表样本，用户也可选择 1 至 10000 个，或在
可选文件不超过 10000 个时选择全部数据。实现先按文件大小稳定排序，再在完整序列上等距
取样，样本数为 `min(用户选择数, 可选文件数)`；因此四个大小区间都能按实际数据占比进入
画像。扩大的是只读画像范围，不会改变 PCAP 检测任务每批最多 20 个文件的限制。
