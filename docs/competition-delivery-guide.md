...
- `/analyze`：Prompt 安全分析工作区，可在“检测分析”和“安全事件”之间切换；查看语义等级、Entropy-CPD 曲线、异常起点、融合动作和脱敏审计记录。
- `/evaluation`：查看冻结评测、工作点、分域结果和局限说明。
- `/events`：兼容入口，打开 Prompt 安全分析工作区的“安全事件”视图。
- `/lab?surface=prompt` 与 `/lab?surface=pcap`：彼此独立的 Prompt 和 PCAP 攻防实验入口，页面内部不跨类型切换。
- `/challenge`：Token 侦探挑战。依次查看三个角色的脱敏报告，再提交判断；已完成角色可重复查看，不会重复执行检测。
- `/pcap-challenge`：PCAP 侦探挑战。复用真实隔离检测、Packet 定位与检测小队，不使用虚构得分。
...
1. 确认本机 PCAP 后端、Docker 与 inspector 镜像已就绪，然后打开 `/lab?surface=pcap`。
...
3. PCAP 实验请单独打开 `/lab?surface=pcap`，避免与 Prompt 输入和结果混在同一页面。
4. 知识报告只引用真实 `knowledge_id`；知识层失败时回退到确定性模板，不改变基础动作。

### PCAP 单文件检测

1. 打开 `/lab?surface=pcap`。
