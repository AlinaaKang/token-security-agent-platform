...
> 本文保留最初 CPD 与 NLL 基线的 663 条研究实验。最新 1,469 条、三智能体链路、838 条冻结 test 的正式消融结果见 [智能体冻结消融实验报告](ablation-report.md)。两份报告回答的问题不同，不应横向拼接指标。
...
后续 `agent-ablation-v1` 将数据扩展为 1,469 条，并在 838 条冻结 test 上比较 Semantic-only、CPD-only 和 Fusion 的生产、Dev FPR 10%、Dev FPR 5% 三类工作点。正式结果表明 Semantic-only 已经很强；Fusion 没有全面改善 F1/FPR，CPD 的主要独立贡献是变化证据和字符级起点定位，而不是总体分类优势。完整来源、哈希、九个工作点、分域结果和限制以 [消融报告](ablation-report.md) 为准。
