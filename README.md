...
PCAP 分层侦察同样是阶段门禁，不是攻击检测器。最小文件优先的 triage 不能刻画整个语料库；
侦察仅使用大小四分位中点抽样和聚合 schema（每个四分位最多五个样本，位置为
`floor((k+0.5)*m/min(5,m))`），完整文件解析只能在现有 Docker 隔离器内执行。
每次真实运行都必须由全新的 UI 授权触发，文件名/路径不能作为输入或标签；观察结果只用于
选择下一版检测器计划（成功样本不足 16 个先修复并重跑，明文不少于 8 个规划 Request，
顺序候选不少于 12 个规划行为检测，加密不少于 12 个规划元数据检测）。报告必须遵循
[PCAP 侦察报告模板](docs/pcap-reconnaissance-report-template.md)，
不得包含逐文件行或私有字段。
...
- 后端：758 passed；1 个本机真实 GPU 集成测试因未配置模型而 skipped，3 个 Windows 符号链接用例因当前账户无相应权限而 skipped；无需该权限的目录 junction 边界测试已通过。
- 前端：229 个自动化测试，TypeScript 与 Vite 生产构建通过。
...
不把聚合结果表述为正式 benchmark，也不把网络证据表述为 Prompt、Token 或 jailbreak 检测。
